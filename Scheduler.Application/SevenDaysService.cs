...
public class SevenDaysService
{
    ...
    { 
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            var block = new ScheduleBlock(range, "Morgonpass", Studio.Studio1);
            day.AddBlock(block);
        ...
        var BlockDay3 = new ScheduleBlock(RangeDay3, "Live session", Studio.Studio1);
        Day3.AddBlock(BlockDay3);

        // Seed with one block on day 6
        var Day6 = days[5];
        var RangeDay6 = new TimeOfDayRange(new TimeOnly(0, 0), new TimeOnly(3, 0));
        var BlockDay6 = new ScheduleBlock(RangeDay6, "Jazz Night", Studio.Studio2);
