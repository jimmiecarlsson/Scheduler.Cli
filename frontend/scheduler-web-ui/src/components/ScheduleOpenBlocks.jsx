...
import { getAll, addPresenter, createPayment } from '../api/scheduleApi';

import BsModal from "../components/BsModal";


function ScheduleOpenBlocks({ user }) {
    ...
    const handleSelectBlock = async (blockId) => {
        ...
        try {
            // Skapa presenter
            await addPresenter(blockId, user.displayName);

            // Skapa payment
            const month = new Date().toISOString().slice(0, 7);
            await createPayment(blockId, month);

            alert("Du har valt blocket och payment skapades!");
            refreshData();
                                ...
                                <ListGroup.Item key={block.id} className="list-group-item-action">
                                    ...
                                    <div className="d-flex align-items-center">
                                        ...
                                        <Button
                                            className="ms-auto"
                                            variant={
                                                !block.presenters || block.presenters.length === 0
                                                    ? "success"
                                                    : "secondary"
                                            }
                                            disabled={block.presenters?.length > 0}
...
export default ScheduleOpenBlocks
